...
void Convert(SqStack equation, char input[], char res[])
{
    ...
    while (ch != '\0')
    {
        if (!(input[i] == '-' || input[i] == '0' || input[i] == '1' || input[i] == '|' || input[i] == '*' || input[i] == '(' || input[i] == ')'))
            break;
        switch (input[i])
        {
        case '(':
        ...
        case '|':
            ...
            break;
        case '*':
            while (!StackEmpty(equation) && GetTop(equation, temp) && (temp == '*' || temp == '-'))
            {
                Pop(equation, temp);
                res[count++] = temp;
            }
...
void judge(char input[], int n, char output[])
{
    switch (n)
    {
    case 1:
...
int main(int argc, char const *argv[])
{
    ...
    char input[MAXSIZE], res[MAXSIZE], output[MAXSIZE];
    ...
    for (int i = 1; i <= 4; i++)
    {
        ...
        judge(input, i, output);
        print(output);
        Convert(equation, output, res);
        print(res);
        result[i] = Calculate(equation, equation_num, res);
    }
