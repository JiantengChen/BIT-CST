...
/*
[Server](https://redisdb/blob/master/database/database.go) is an implemention of interface [DB](https://redisdb/blob/master/interface/database/db.go).

[server.Handler](https://redisdb/blob/master/redis/server/server.go) holds an instance of Server as storage engine, and pass command line to Server through db.Exec method.
...
[godis.DB.Exec](https://redisdb/blob/master/database/single_db.go) handles transaction control command (such as watch, multi, exec) itself, and invokes DB.execNormalCommand to handle normal commands. The word, normal command, is commands which read or write limited keys, can execute within transaction, and supports rollback. For example, get, set, lpush are normal commands, while flushdb, keys are not.

[registerCommand](https://redisdb/blob/master/database/router.go) is used for registering normal command. A normal command requires three functions：

- ExecFunc: The function that actually executes the command, such as [execHSet](https://redisdb/blob/master/database/hash.go)
